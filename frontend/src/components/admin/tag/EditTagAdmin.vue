  ...
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      <li class="breadcrumb-item active" aria-current="page">
        <span class="text-black-50">Home / Tags / Editar Tag</span>
      </li>
    </ol>
  </nav>
...
export default {
  ...
  methods: {
    async getTag() {
      ...
      try {
        const response = await this.TagAdminService.getTag(id);
    ...
    async editTag(data) {
      ...
      const response = await this.TagAdminService.updateTag(data);
